    ...
    (function (SWF) {
        ...
        (function (Parser) {
            ...
            function defineFont(tag) {
                ...
                var code;
                if (tag.codes) {
                    for (var i_1 = 0; i_1 < tag.codes.length; i_1++) {
                        var code_1 = tag.codes[i_1];
                    ...
                    var i_2 = 0;
                    while ((code = codes[i_2++]) !== undefined) {
                        var start = code;
                        var end = start;
                        indices = [i_2 - 1];
                        while (((code = codes[i_2]) !== undefined) && end + 1 === code) {
                            ++end;
                            indices.push(i_2);
                            ++i_2;
                        }
                    ...
                    for (var i_3 = 0; i_3 < glyphCount; i_3++) {
                        code = UAC_OFFSET + i_3;
                        codes.push(code);
                        glyphIndex[code] = i_3;
                        indices.push(i_3);
                ...
                for (var i_4 = 0; i_4 < glyphCount; ++i_4) {
                    hmtx += toString16(advance ? (advance[i_4] / resolution) : 1024) + '\x00\x00';
                }
                tables['hmtx'] = hmtx;
                ...
                for (var i_6 = 0; i_6 < otf.length; i_6++) {
                    dataBuffer[i_6] = otf.charCodeAt(i_6) & 0xff;
                }
        ...
        (function (Parser) {
            ...
            function parseJpegChunks(bytes, chunks) {
                ...
                do {
                    ...
                    if ((code >= 0xc0 && code <= 0xc7) || (code >= 0xc9 && code <= 0xcf) ||
                        ...
                        var length_2 = readUint16(bytes, i);
                        i += length_2;
                    }
                    // Finding next marker.
                    while (i < n && (bytes[i] !== 0xff ||
            ...
            function defineImage(tag) {
                ...
                if (tag.mimeType === 'image/jpeg') {
                    ...
                    if (alphaData) {
                        var length_3 = image.width * image.height;
                        try {
                            image.alphaData = Inflate.inflate(alphaData, length_3, true);
                        }
                        catch (e) {
                            // Alpha layer is invalid, so hiding everything.
                            image.alphaData = new Uint8Array(length_3);
                        }
                    }
                }
                else {
            ...
            SWFFile.prototype.scanTag = function (tag, rootTimelineMode) {
                ...
                switch (tagCode) {
                    ...
                    case 56 /* CODE_EXPORT_ASSETS */:
                        ...
                        var exports_1 = this._currentExports || (this._currentExports = []);
                        while (exportsCount--) {
                            var symbolId = stream.readUi16();
                            ...
                            exports_1.push(new SymbolExport(symbolId, className));
                        }
                        stream.pos = tagEnd_2;
                        break;
