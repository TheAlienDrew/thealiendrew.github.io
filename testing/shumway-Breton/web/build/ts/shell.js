...
(function (Shumway) {
    ...
    (function (Shell) {
        ...
        var MicroTasksQueue = /** @class */ (function () {
...
(function (Shumway) {
    var Shell;
    (function (Shell) {
        var ShellBinaryFileReader = /** @class */ (function () {
...
if (isV8) {
    ...
    read = function (path, type) {
        return type === 'binary' ? new Uint8Array(readbuffer(path)) : oldread_1(path);
    };
}
else if (isJSC) {
...
(function (Shumway) {
    ...
    (function (Shell) {
        ...
        var ShellGFXServer = /** @class */ (function () {
        ...
        function main(commandLineArguments) {
            ...
            if (compileOption.value) {
                ...
                files.forEach(function (file) {
                    ...
                    if (file.endsWith(".abc")) {
                        ...
                        buffers_1.push.apply(buffers_1, extractABCsFromSWF(buffer));
                    }
                });
                Shell.verbose && writer.writeLn("Loading " + buffers_1.length + " ABCs");
            ...
            if (executeOption.value) {
                var shouldLoadPlayerGlobalCatalog_1 = loadPlayerGlobalCatalogOption.value;
                if (!shouldLoadPlayerGlobalCatalog_1) {
                    // We need to load player globals if any swfs need to be executed.
                    files.forEach(function (file) {
                        if (file.endsWith(".swf")) {
                            shouldLoadPlayerGlobalCatalog_1 = true;
                        }
                    });
                }
                executeFiles(files);
                ...
                var sec_1 = createSecurityDomain(builtinABCPath, null, null);
                files.forEach(function (file) {
                    if (file.endsWith(".abc")) {
                        disassembleABCFile(sec_1, file);
                    }
                });
            }
            if (errors) {
        ...
        function executeSWFFile(file, runDuration, runCount, frameCount) {
            ...
            try {
                var hash_1 = 0;
                var lastFramesPlayed_1 = 0;
                writer.writeLn("RUNNING:  " + file);
                microTaskQueue.run(runDuration, runCount, true, function () {
                    if (!frameCount) {
                        return true;
                    }
                    if (lastFramesPlayed_1 < player.framesPlayed) {
                        hash_1 = Shumway.HashUtilities.mixHash(hash_1, player.stage.hashCode());
                        ...
                        lastFramesPlayed_1 = player.framesPlayed;
                    }
                ...
                writer.writeLn("HASHCODE: " + file + ": " + Shumway.IntegerUtilities.toHEX(hash_1));
            }
            catch (x) {
